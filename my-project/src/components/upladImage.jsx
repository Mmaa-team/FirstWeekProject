...
const UploadImage = ({ change, setUploaded, uploaded }) => {
    ...
    const addPost = async (e) => {
        ...
        const downloadurl = await getDownloadURL(imageRef)
        await axios
            .post(`http://localhost:8080/posts/bgimage/1`, {
                bgImage: downloadurl,
            })
            .then(() => {
                setUploaded(!uploaded)
            ...
            <div className="flex items-center justify-center p-12 backdrop-blur-lg">
                <div className="mx-auto w-full max-w-[550px] bg-white">
                    <form className="px-9 py-6">
                        ...
                        <div>
                            <input className=" w-50 mb-4 mt-2 rounded-md border-0 bg-gray-100 p-2 text-gray-800 transition duration-150 ease-in-out focus:bg-gray-200 focus:outline-none focus:ring-1 focus:ring-Liver" type="text" onChange={(e)=>{setPost(e.target.value)}}></input>
                        </div>
                        <div>
                            <button
                                onClick={async (e) => {
